...
def test_stream_script_chat_reply_yields_chunks_in_order(monkeypatch):
    fake = _FakeAgent(
        [
    ...
    async def _collect():
        return [
            chunk
            async for chunk in script_chat_agent.stream_script_chat_reply(
...
def test_stream_script_chat_reply_skips_chunks_with_empty_content(monkeypatch):
    fake = _FakeAgent(
        [
...
def test_stream_script_chat_reply_yields_reasoning_before_content(monkeypatch):
    fake = _FakeAgent(
        [
            {
    ...
    async def _collect():
        return [
            item
            async for item in script_chat_agent.stream_script_chat_reply(
                [{"role": "user", "content": "Hi"}],
                project="TestProject",
            )
        ]
...
def test_stream_script_chat_reply_passes_provider_and_model(monkeypatch):
    captured = {}
...
def test_build_system_prompt_default_forbids_extending_recorded_roles():
    prompt = script_chat_agent._build_system_prompt(False)
    assert "KEINE neuen Zeilen" in prompt
    assert "genau wie bei jeder anderen Sprechrolle" not in prompt


def test_build_system_prompt_can_allow_extending_recorded_roles():
    prompt = script_chat_agent._build_system_prompt(True)
    assert "genau wie bei jeder anderen Sprechrolle" in prompt
    assert "KEINE neuen Zeilen" not in prompt


def test_stream_script_chat_reply_passes_allow_extend_recorded_roles(monkeypatch):
    captured = {}
