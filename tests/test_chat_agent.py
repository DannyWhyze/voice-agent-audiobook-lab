...
def test_stream_chat_reply_yields_chunks_in_order(monkeypatch):
    fake = _FakeAgent(
        [
    ...
    async def _collect():
        return [
            chunk
            async for chunk in chat_agent.stream_chat_reply(
...
def test_stream_chat_reply_skips_chunks_with_empty_content(monkeypatch):
    fake = _FakeAgent(
        [
...
def test_stream_chat_reply_yields_reasoning_before_content(monkeypatch):
    fake = _FakeAgent(
        [
            {
    ...
    async def _collect():
        return [
            item
            async for item in chat_agent.stream_chat_reply(
                [{"role": "user", "content": "Hi"}],
                project="TestProject",
            )
        ]
...
def test_stream_chat_reply_passes_provider_and_model(monkeypatch):
    captured = {}
    ...
    monkeypatch.setattr(chat_agent, "_build_agent", _fake_build)
