...
export function setProjectChatHistory(newHistory) {
  history = newHistory || [];
  reasoningByIndex = new Map();
}

export function openProjectChatOverlay({ t, chatUrl, getProject, onHistoryChanged, onMessageComplete, onClosed }) {
  ...
  title.textContent = "Bars2Bars Projekt-Chat";
  ...
  const stopResizing = makeResizable(panel, resizeHandle, "projectChat");
  ...
  resetBtn.textContent = t("projectChatReset");
