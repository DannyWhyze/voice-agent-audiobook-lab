...
export function openFormantOverlay({ t, project, previewUrl, applyUrl, onApplied, initialParams, onClosed }) {
  const params = {
    semitones: 0,
    cents: 0.0,
  };
  ...
  const liveAudio = createSemitoneRatioLiveAudio({
    params,
    createShifter: (framerate) => new FormantShifter(framerate),
    setRatio: (shifter, ratio) => shifter.setFormantRatio(ratio),
  });
  ...
  title.textContent = "Bars2Bars Formant";
  ...
  const stopResizing = makeResizable(panel, resizeHandle, "formant");

  const { semitonesInput, centsInput } = buildSemitoneCentsRows({
    t,
    panel,
    params,
    semitonesLabelKey: "formantSemitonesLabel",
    centsLabelKey: "formantCentsLabel",
